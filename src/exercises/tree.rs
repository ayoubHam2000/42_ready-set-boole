...
impl Node {
  ...
  pub fn as_string(&self) -> String {
    match self.value {
      ...
      Token::MaterialCondition |
      Token::ExclusiveDisjunction => {
        let a = self.left.as_ref().unwrap().borrow().as_string();
        let b = self.right.as_ref().unwrap().borrow().as_string();
        format!("({} {} {})", a, self.value.get_value(), b)
      },
      Token::Negation => {
        let a = self.left.as_ref().unwrap().borrow().as_string();
        format!("!{}", a)
  ...
  pub fn as_string_colored(&self, counter : usize) -> String {
    ...
    match self.value {
      ...
      Token::MaterialCondition |
      Token::ExclusiveDisjunction => {
        let a = self.left.as_ref().unwrap().borrow().as_string_colored(counter + 1);
        let b = self.right.as_ref().unwrap().borrow().as_string_colored(counter + 1);
...
impl Tree {
  ...
  pub fn as_string(&self) -> String {
    if let Some(root) = self.root.as_ref() {
      let res = root.borrow().as_string();
      return res;
    }
    panic!("root is not initialized");
  }

  pub fn print(&self) {
    if let Some(root) = self.root.as_ref() {
      let res = root.borrow().as_string_colored(0);
      println!("{}", res);
    } else {
      println!("None")
    }
